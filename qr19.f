...
C  Lower mantle value from PREM
	...
	save dad,tad,nad
     ...
     &       ,153.5,17.1,17.1/
        data ncall/0/
        ncall = ncall + 1
        if (ncall .eq. 1) write(31,*) 'Base Q model: QR19'
        if (ncall .eq. 1) write(31,*) 'Age for Q computation (Ma) = ',age
        if (ncall .eq. 1) then
         fname = 'ad1600.inv251010'
         write(31,*) 'Reference temperature profile for Q computation = ',fname
         open(77,file=fname,status='old')
         nad = 0
         do 1 i=1,nadmax
          read(77,*,end=10) pad,dad(i),tad(i)
          nad = nad + 1
1        continue
10       continue
