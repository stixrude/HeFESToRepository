...
c	print*, 'In volumew entering cage and pressurew',vlow,vupp
c        call zbrac(pressurew,x1,x2,succes)
	call cage(pressurew,x1,x2,vlow,vupp,ires)
c	print*, 'back from zbrac in volumew',succes,x1,x2
	p1 = pressurew(x1)
	p2 = pressurew(x2)
	plow = pressurew(vlow)
	pupp = pressurew(vupp)
c	print*, 'back from cage in volumew',x1,x2,vlow,vupp,p1,p2
c        if (succes) then
        if (ires .eq. 1) then
c         volumew = zbrent(pressurew,x1,x2,tol)
         volumew = zeroin(x1,x2,pressurew,tol)
c	 print*, 'Found volume',volumew
        else
